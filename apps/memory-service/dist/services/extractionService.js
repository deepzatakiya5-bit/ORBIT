import { extractionOutputSchema } from "../types/memory.js";
...
export async function processConversationMemories(args) {
    ...
    const extraction = extractionOutputSchema.parse(await generateJson(prompt));
...
async function upsertMemory(args, mem) {
    ...
    await args.prisma.$executeRawUnsafe(`INSERT INTO memory_embedding (id, memory_id, provider, dimensions, embedding, created_at, updated_at)
     VALUES (gen_random_uuid(), $1, 'gemini', $2, $3::vector, now(), now())
     ON CONFLICT (memory_id) DO UPDATE SET embedding = excluded.embedding, updated_at = now()`, created.id, embedding.length, `[${embedding.join(",")}]`);
}
