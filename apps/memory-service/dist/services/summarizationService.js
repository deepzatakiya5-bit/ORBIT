...
export async function generateDailySummary(prisma, userId, day) {
    ...
    const payload = await generateJson(buildSummaryPrompt({ period: "DAILY", slices }));
...
export async function rollupSummary(prisma, userId, period, anchor) {
    if (period === "WEEKLY") {
        ...
        if (!days.length)
            return;
        const payload = await generateJson(buildSummaryPrompt({ period: "WEEKLY", slices: days.map((d) => d.summary) }));
    ...
    if (period === "MONTHLY") {
        ...
        if (!weeks.length)
            return;
        const payload = await generateJson(buildSummaryPrompt({ period: "MONTHLY", slices: weeks.map((w) => w.summary) }));
    ...
    if (period === "YEARLY") {
        ...
        if (!months.length)
            return;
        const payload = await generateJson(buildSummaryPrompt({ period: "YEARLY", slices: months.map((m) => m.summary) }));
