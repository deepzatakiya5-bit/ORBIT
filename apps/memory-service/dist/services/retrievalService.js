...
export async function retrieveMemories(prisma, input) {
    const limit = input.limit ?? 20;
    const now = input.now ?? new Date();
    const vector = await embedText(input.query);
    const vectorLiteral = `[${vector.join(",")}]`;
    const rows = await prisma.$queryRawUnsafe(`SELECT m.id, m.content, m.type::text, m.salience, m.confidence, m.decay_score, m.updated_at,
