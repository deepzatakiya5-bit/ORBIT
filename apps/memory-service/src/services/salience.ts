...
export function recencyScore(lastSignalAt: Date, now = new Date()) {
  const days = Math.max(0, dayjs(now).diff(lastSignalAt, "day"));
  return Math.exp(-days / 45);
}

export function decayHalfLifeDays(type: MemoryType): number {
  switch (type) {
...
export function reinforceMemory(currentSalience: number, strength = 0.08) {
